	...
	network.on('select', function(params) {
        ...
        if(params.nodes.length > 0){
          if(findArea(nodes.get(params.nodes)[0].label) != null){
            var node = findArea(nodes.get(params.nodes)[0].label);
            $('#selection').append("Area: " + node.areaName +
            "<button type=\"button\" id=\"edit-area-button\" class=\"btn btn-primary btn-sm btn-block\">Edit Area</button>");
          }
          else {
            var node = findACU(nodes.get(params.nodes)[0].label, findArea(getNodeArea(nodes.get(params.edges)[0].id)));
            $('#selection').append("Area: " + node.area + "\t ACU: " + node.acuName +
            "<button type=\"button\" id=\"edit-device-button\" class=\"btn btn-primary btn-sm btn-block\">Edit Device</button>");
          }
        }
